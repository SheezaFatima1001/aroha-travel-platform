...
export const createReview = async (req, res, next) => {
  try {
    ...
    if (!validTarget(targetType)) return res.status(400).json({ success: false, message: 'targetType must be Destination or Service' });
    if (rating === undefined || rating === null) return res.status(400).json({ success: false, message: 'Rating is required' });
    if (Number(rating) < 1 || Number(rating) > 5) return res.status(400).json({ success: false, message: 'Rating must be between 1 and 5' });
    if (!reviewText || !reviewText.trim()) return res.status(400).json({ success: false, message: 'Review text is required' });
    ...
    if (existing) return res.status(400).json({ success: false, message: 'You have already reviewed this. Edit your existing review instead.' });

    const review = await Review.create({ user: req.user._id, targetType, targetId, rating, reviewText: reviewText.trim(), image: image || '' });
    const populated = await review.populate('user', 'name profileImage');
    res.status(201).json({ success: true, data: populated });
  } catch (err) {
    if (err.code === 11000) return res.status(400).json({ success: false, message: 'You have already reviewed this.' });
    next(err);
  }
};
...
export const getReviewSummary = async (req, res, next) => {
  try {
    ...
    if (!agg) {
      return res.json({ success: true, data: { averageRating: 0, totalReviews: 0, distribution: { 5: 0, 4: 0, 3: 0, 2: 0, 1: 0 } } });
    }
    res.json({
      success: true,
      data: {
        ...
        distribution: { 5: agg.fiveStar, 4: agg.fourStar, 3: agg.threeStar, 2: agg.twoStar, 1: agg.oneStar },
      },
    });
  } catch (err) {
    next(err);
...
export const getMyReviews = async (req, res, next) => {
  try {
    const reviews = await Review.find({ user: req.user._id }).populate('user', 'name profileImage').populate('targetId').sort({ createdAt: -1 });
    res.json({ success: true, count: reviews.length, data: reviews });
  } catch (err) {
    next(err);
  }
...
export const updateReview = async (req, res, next) => {
  try {
    ...
    if (!review) return res.status(404).json({ success: false, message: 'Review not found' });
    if (review.user.toString() !== req.user._id.toString()) return res.status(403).json({ success: false, message: 'Not authorized to edit this review' });
    ...
    if (reviewText !== undefined) {
      if (!reviewText.trim()) return res.status(400).json({ success: false, message: 'Review text is required' });
